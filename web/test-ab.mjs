...
(async () => {
  ...
  try {
    ...
    const draftsResult = await page.evaluate(() => {
      // Import the function from the module context
      if (typeof window !== 'undefined') {
        // We can't directly import in browser context, so let's check what's displayed
      ...
      return { count: -2, message: 'Not browser context' };
    });
