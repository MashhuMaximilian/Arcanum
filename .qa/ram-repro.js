 ...
 * Usage: node .qa/ram-repro.js
 */

const { chromium } = require('/Users/max/dev/Arcanum/web/node_modules/playwright');
const fs = require('fs');
...
async function getMemorySample(page) {
...
async function takeHeapSnapshot(page, label) {
  var cdp = await page.context().newCDPSession(page);
  try {
    var snapshot = await cdp.send('HeapProfiler.takeHeapSnapshot', {});
    var snapshotStr = JSON.stringify(snapshot);
    var filePath = path.join(QA_DIR, 'heap-' + label + '-' + getTimestamp() + '.heapsnapshot');
    fs.writeFileSync(filePath, snapshotStr);
    console.log('  [Snapshot] Saved ' + filePath + ' (' + (snapshotStr.length / 1024 / 1024).toFixed(2) + ' MB)');
  } catch (e) {
...
async function main() {
  ...
  // Check dev server
  try {
    var checkResponse = await fetch(DEV_URL);
    console.log('Dev server status: ' + checkResponse.status);
  } catch (e) {
    console.log('Dev server check failed: ' + e);
  }
  console.log('');
  ...
  // Seed localStorage BEFORE first navigation via addInitScript
  console.log('=== Seeding localStorage via addInitScript ===');
  var corruptedPayload = buildCorruptedDrafts();
  context.addInitScript(function(payload) {
  ...
  var mem1 = await getMemorySample(page);
  ...
  var mem2 = await getMemorySample(page);
  var console1count = consoleLogs.length;
  ...
  console.log('Console messages: ' + console1count);

  await takeHeapSnapshot(page, 'phase1-after-load');

  // Screenshot
  var screenshot1Path = path.join(QA_DIR, 'characters-state-phase1-' + getTimestamp() + '.png');
  await page.screenshot({ path: screenshot1Path });
  console.log('Screenshot: ' + screenshot1Path);

  // Check page content
  var draftCount = await page.evaluate(function() {
    var rows = document.querySelectorAll('[data-testid*="draft"], .draft, [class*="draft"]');
    return rows.length;
  });
  console.log('Visible draft elements: ' + draftCount);

  var pageErrors = consoleLogs.filter(function(l) { return l.indexOf('[error]') !== -1 || l.indexOf('Error') !== -1; });
  ...
  var mem3 = await getMemorySample(page);
  ...
  var mem4 = await getMemorySample(page);
  var console2count = consoleLogs.length;
  ...
  var screenshot2Path = path.join(QA_DIR, 'characters-state-phase2-' + getTimestamp() + '.png');
  ...
  if (mem4.jsHeap > 10 * 1024 * 1024 * 1024) {
    console.log('\nSEVERE: Memory exceeds 10GB — spike confirmed!');
  } else if (mem4.jsHeap > 500 * 1024 * 1024) {
    console.log('\nWARNING: Memory at ' + (mem4.jsHeap / 1024 / 1024).toFixed(0) + ' MB — elevated but not 15GB');
  } else {
    console.log('\nOK: Memory appears stable');
  }

  // Save full console log
  var logPath = path.join(QA_DIR, 'console-log-' + getTimestamp() + '.txt');
